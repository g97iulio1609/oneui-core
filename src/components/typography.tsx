...
const typographyVariants = cva('text-neutral-900 dark:text-neutral-100', {
  variants: {
    variant: {
      ...
      blockquote: 'mt-6 border-l-2 border-neutral-300 pl-6 italic text-neutral-800 dark:border-neutral-600 dark:text-neutral-200',
...
export interface TypographyProps
  extends React.HTMLAttributes<HTMLHeadingElement | HTMLParagraphElement | HTMLQuoteElement | HTMLUListElement>,
...
export const Typography = React.forwardRef<HTMLElement, TypographyProps>(
  ({ className, variant, weight, align, as: Component, ...props }, ref) => {
    ...
    const Comp = Component || (
      ...
      variant === 'h3' ? 'h3' :
      variant === 'h4' ? 'h4' :
      variant === 'p' || variant === 'lead' || variant === 'large' || variant === 'muted' || variant === 'error' ? 'p' :
